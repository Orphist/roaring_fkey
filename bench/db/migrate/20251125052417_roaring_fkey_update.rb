class RoaringFkeyUpdate < ActiveRecord::Migration[6.1]
  def up
    execute <<~SQL
      ...
      DROP FUNCTION IF EXISTS roaring_fkey_bigint_contains_in_bitmap64(bigint, roaringbitmap64) CASCADE;
      ...
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap64_count(bigint, roaringbitmap64) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap64_max(bigint, roaringbitmap64) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap64_min(bigint, roaringbitmap64) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_contains_bigint64(roaringbitmap64, bigint) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_contains_int(roaringbitmap, integer) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_count(integer, roaringbitmap) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_max(integer, roaringbitmap) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_min(integer, roaringbitmap) CASCADE;
      DROP FUNCTION IF EXISTS roaring_fkey_bitmap_overlaps_array_bigint64(roaringbitmap64, bigint[]) CASCADE;
    ...
    execute <<~SQL
      ...
      begin
        ...
        CREATE FUNCTION roaring_fkey_bitmap_overlaps_array_bigint64(roaringbitmap64, bigint[])
        returns boolean language sql immutable as $$
          SELECT $1 && (rb64_build($2));
        $$;

        CREATE OPERATOR && (leftarg = roaringbitmap64, rightarg = bigint[],
            procedure = roaring_fkey_bitmap_overlaps_array_bigint64,
            commutator = &&);
      ...
      exception
          when duplicate_function then
               null;
          when others then
            raise notice E'Got exception: func roaring_fkey_bitmap_overlaps_array_bigint64';
