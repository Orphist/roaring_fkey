...
RSpec.describe 'RoaringBitmap64', :aggregate_failures, :db do
  ...
  describe 'associations' do
    let(:model_class) do
      Class.new(ActiveRecord::Base) do
        self.table_name = 'test_roaringbitmap64_models'
        belongs_to_many :items, class_name: 'TestAssociatedModel', foreign_key: 'item_ids'
      end
    end
    ...
    before do
      # Create associated records with bigint IDs
      @item1 = associated_class.create!(name: 'Item 1', id: 1000000001)
      @item2 = associated_class.create!(name: 'Item 2', id: 1000000002)
      @item3 = associated_class.create!(name: 'Item 3', id: 1000000003)
    end

    it 'can create association with bigint foreign keys' do
      record = model_class.create!(name: 'test', items: [@item1, @item2])
      expect(record.items).to include(@item1)
      expect(record.items).to include(@item2)
      expect(record.items).not_to include(@item3)
    end

    it 'can add items to association' do
      record = model_class.create!(name: 'test', items: [@item1])
      record.items << @item2
      record.reload
      expect(record.items).to include(@item1, @item2)
    end

    it 'can remove items from association' do
      record = model_class.create!(name: 'test', items: [@item1, @item2, @item3])
      record.items.delete(@item2)
      record.reload
      expect(record.items).to include(@item1, @item3)
      expect(record.items).not_to include(@item2)
    end

    it 'can replace items in association' do
      record = model_class.create!(name: 'test', items: [@item1])
      record.items = [@item2, @item3]
      record.reload
      expect(record.items).to include(@item2, @item3)
      expect(record.items).not_to include(@item1)
    end
  end
end