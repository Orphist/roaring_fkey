...
module Casting64
  ...
  def cast_to_array(value)
    ...
    value_sql = <<~SQL
      SELECT rb64_to_array(#{quoted_value});
    SQL
  ...
  def cast_from_array(value)
    ...
    value_sql = <<~SQL
      SELECT rb64_build(ARRAY[#{values.join(',')}]);
    SQL
        ...
        class Roaringbitmap64 < ActiveModel::Type::Value
          ...
          def deserialize(value)
            ...
            when ::Numeric
              ...
              raise NotImplementedError, "deserialize(value):Don't know how to cast #{value.class} #{value.inspect} into Roaringbitmap64"
            end
          end
          ...
          def cast(value)
            ...
            case value
            when ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Roaringbitmap64
            ...
            when ::Array
              ...
              raise NotImplementedError, "cast(value):Don't know how to cast #{value.class} #{value.inspect} into Roaringbitmap64"
            end
          end
          ...
          def cast_value(value)
            ...
            when String
              ...
              raise NotImplementedError, "cast_value(value):Don't know how to cast #{value.class} #{value.inspect} into Roaringbitmap64"
            end
          end
...
PostgreSQLAdapterWithRoaringbitmap64 = Module.new do
  def initialize_type_map(m = type_map)
    m.register_type "roaringbitmap64" do |*_args, _sql_type|
      ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Roaringbitmap64.new
    end
    m.alias_type "_roaringbitmap64", "roaringbitmap64"

    # Call Rails logic after ours or it will complain that OID isn't supported
    super
  end
end
ActiveRecord::ConnectionAdapters::PostgreSQLAdapter.prepend(PostgreSQLAdapterWithRoaringbitmap64)
ActiveRecord::Type.register(:roaringbitmap64, ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Roaringbitmap64,
                            adapter: :postgresql)
...
module SchemaStatementsWithRoaringbitmap64
  def type_to_sql(type, limit: nil, precision: nil, scale: nil, array: nil, **)
    case type.to_s
    when 'roaringbitmap64' then 'roaringbitmap64'
    else super
    end
  end
end
ActiveRecord::ConnectionAdapters::PostgreSQL::SchemaStatements.prepend(SchemaStatementsWithRoaringbitmap64)