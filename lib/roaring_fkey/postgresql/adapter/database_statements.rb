    ...
    module Adapter
      module DatabaseStatements
        ...
        def schemas_blacklist
          @schemas_blacklist ||= RoaringFkey::PostgreSQL.config.schemas.blacklist +
            (@config.dig(:schemas, 'blacklist') || [])
        end

        # List of schemas used by the application in the current connection
        def schemas_whitelist
          @schemas_whitelist ||= RoaringFkey::PostgreSQL.config.schemas.whitelist +
        ...
        def load_additional_types(oids = nil)
          super
          roaring_pg_load_additional_types(oids)
        end

        # Add the composite types to be loaded too.
        def roaring_pg_load_additional_types(oids = nil)
